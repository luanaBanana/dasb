...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("Student Exam Scores"),
    ...
    mainPanel(
        tabsetPanel(
            tabPanel("Score Prediction", 
                ...
                selectInput(inputId = "prep", label = "Did you complete a test preparation course", choice = c("completed", "none")),
                
                selectInput(inputId = "gender", label = "Choose your gender", choices = c("male", "female")),
                
                selectInput(inputId = "race", label = "Select your ethnic group", choices = c("group A", "group B", "group C", "group D", "group E")),
                textOutput("distPlot")
                ), 
...
server <- function(input, output) {
    output$distPlot <- renderText({ 
        ...
        paste("The average score for you input parameters are: Math ", result[1], ", Reading  ", result[2], ", Writing", result[3], ".")
    })
    
    output$marksummary<-renderPlot({
        if (input$type == "writing"){
            ggplot(studPerf, aes(studPerf$gender, studPerf$`writing score`, color = `test preparation course`)) + 
                geom_boxplot() + 
                ggtitle("Writing scores by Gender Boxplot") + 
                xlab("Gender") + 
                ylab("Writing Scores")
        } else if (input$type == "reading"){
            ggplot(studPerf, aes(studPerf$gender, studPerf$`reading score`, color = `test preparation course`)) + 
                geom_boxplot() + 
